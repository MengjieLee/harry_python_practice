'''
/bigram_v5_with_FFN.py
step(0): train loss 6.7144, val loss 6.7609
step(500): train loss 1.4049, val loss 10.3505
step(1000): train loss 0.6332, val loss 14.6836
step(1500): train loss 0.4802, val loss 16.4476
step(2000): train loss 0.4005, val loss 16.8121
step(2500): train loss 0.3742, val loss 17.4359
step(3000): train loss 0.3570, val loss 17.5010
step(3500): train loss 0.3450, val loss 17.5505
step(4000): train loss 0.3393, val loss 16.9912
step(4500): train loss 0.3269, val loss 16.8589
'''
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        out = torch.cat([h(x) for h in self.heads], dim=-1) # 高维拼接：把多个注意力头的输出沿特征维度拼接
        out = self.proj(out)
        return out
